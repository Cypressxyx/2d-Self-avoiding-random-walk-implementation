...
def checkMove(u,v):
    ...
    if ( u[0] == v and len(u) == 1):
        if (( u.count(v) == nsma)):
            return 0
        else:
            return 1

    distance = getDist(u,v)
    ...
    newU = u[:]
    val = 0
    for i in range(len(newU) - 1, -1,-1):
        val = newU[i]
        newU = newU[:-1]
        sumU += getDist(newU, val)
    repeatedVals = []
    for i in range(0,len(u)):
        if(u[i] == u[0] ):
            repeatedVals.append(u[0])
    if(len(repeatedVals) > 1):
        newU = repeatedVals[:-1]
        vs = 0
        if(oldU == [3,3,1] and v == 3):
            print(newU)
        for i in range(len(newU) - 1 ,-1,-1):
            val = newU[i]
            newU = newU[:-1]
